...
public class Mapper
{
    ...
    private static int CalcSOF(List<DriverModel> drivers)
    {
        ...
        // iRating calculation for each driver
        foreach (var result in drivers)
        {
            double expectedScore = 0;
            foreach (var other in drivers)
            {
                if (result.Idx == other.Idx) continue;

                // Probability of beating 'other' driver
                double p = (1.0 - Math.Exp(-result.iRating / LogNumber)) * Math.Exp(-other.iRating / LogNumber) /
                           ((1.0 - Math.Exp(-result.iRating / LogNumber)) * Math.Exp(-other.iRating / LogNumber) +
                            (1.0 - Math.Exp(-other.iRating / LogNumber)) * Math.Exp(-result.iRating / LogNumber));
                
                expectedScore += p;
            }

            // Simple fudge factor to align with iRacing's slightly biased results
            double fudgeFactor = (starters / 2.0 - (result.ClassPosition - 1)) / 100.0;
            
            // Standard gain multiplier for most series is 200 / starters
            // result.ClassPosition is 1-indexed
            result.RatingChange = (starters - result.ClassPosition - expectedScore - fudgeFactor) * 200.0 / starters;
        }
